...
class RuleSimulator(UserSimulator):
    ...
    def __init__(
        self, movie_dict=None, act_set=None, slot_set=None, start_set=None, params=None
    ):
    ...
    def _sample_action(self):
        ...
        if len(self.goal["inform_slots"]) > 0:
            ...
            if (
                "moviename" in self.goal["inform_slots"].keys()
            ):  # 'moviename' must appear in the first user turn
                self.state["inform_slots"]["moviename"] = self.goal["inform_slots"][
                    "moviename"
                ]
    ...
    def corrupt(self, user_action):
        ...
        for slot in user_action["inform_slots"].keys():
            ...
            if (
                slot_err_prob_sample < self.slot_err_probability
            ):  # add noise for slot level
        ...
        if (
            intent_err_sample < self.intent_err_probability
        ):  # add noise for intent level
    ...
    def response_confirm_answer(self, system_action):
        ...
        if len(self.state["rest_slots"]) > 0:
            ...
            if request_slot in self.goal["request_slots"].keys():
                ...
                self.state["diaact"] = "inform"
                self.state["inform_slots"][request_slot] = self.goal["inform_slots"][
                    request_slot
                ]
    ...
    def response_thanks(self, system_action):
        ...
        for info_slot in self.state["history_slots"].keys():
            ...
            if info_slot in self.goal["inform_slots"].keys():
                if (
                    self.state["history_slots"][info_slot]
                    != self.goal["inform_slots"][info_slot]
                ):
                    self.dialog_status = dialog_config.FAILED_DIALOG
    ...
    def response_request(self, system_action):
        ...
        if len(system_action["request_slots"].keys()) > 0:
            ...
            if (
                slot in self.goal["inform_slots"].keys()
                ...
                slot in self.goal["request_slots"].keys()
                ...
                slot in self.goal["request_slots"].keys()
                and slot in self.state["rest_slots"]
                ...
                if (
                    len(self.state["request_slots"]) == 0
                    and len(self.state["rest_slots"]) == 0
                ):
                    self.state["diaact"] = "thanks"
                else:
                    self.state["diaact"] = "inform"
            ...
            if len(self.state["rest_slots"]) > 0:
                ...
                if random_slot in self.goal["inform_slots"].keys():
                    self.state["inform_slots"][random_slot] = self.goal["inform_slots"][
                        random_slot
                    ]
    ...
    def response_inform(self, system_action):
        ...
        if (
            "taskcomplete" in system_action["inform_slots"].keys()
            ...
            if (
                system_action["inform_slots"]["taskcomplete"]
                == dialog_config.NO_VALUE_MATCH
            ):
                        ...
                        if len(self.state["request_slots"]) > 0:
                            ...
                            if len(rest_slot_set) > 0:
                                ...
                                if inform_slot in self.goal["inform_slots"].keys():
                                    self.state["inform_slots"][inform_slot] = self.goal[
                                        "inform_slots"
                                    ][inform_slot]
                                    self.state["diaact"] = "inform"
                    ...
                    if len(self.state["request_slots"]) > 0:
                        ...
                        if len(rest_slot_set) > 0:
                            ...
                            if inform_slot in self.goal["inform_slots"].keys():
                                ...
                                self.state["request_slots"][inform_slot] = self.goal[
                                    "request_slots"
                                ][inform_slot]
                                self.state["diaact"] = "request"
                        else:
