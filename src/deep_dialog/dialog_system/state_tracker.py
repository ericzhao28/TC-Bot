...
class StateTracker:
    ...
    def get_state_for_agent(self):
        ...
        state = {
            ...
            "agent_action": self.history_dictionaries[-2]
            if len(self.history_dictionaries) > 1
            else None,
        }
    ...
    def get_suggest_slots_values(self, request_slots):
        ...
        if len(request_slots) > 0:
            suggest_slot_vals = self.kb_helper.suggest_slot_values(
                request_slots, self.current_slots
            )
    ...
    def update(self, agent_action=None, user_action=None):
        ...
        if agent_action:
            ...
            if agent_action["act_slot_response"]:
                ...
                agent_action_values = copy.deepcopy(
                    agent_action["act_slot_value_response"]
                )
            ...
            for slot in agent_action_values["inform_slots"].keys():
                self.current_slots["proposed_slots"][slot] = agent_action_values[
                    "inform_slots"
                ][slot]
                self.current_slots["inform_slots"][slot] = agent_action_values[
                    "inform_slots"
                ][
                    slot
            ...
            self.history_vectors = np.vstack(
                [self.history_vectors, current_agent_vector]
            )
        ...
        ########################################################################
            ...
            for slot in user_action["inform_slots"].keys():
                self.current_slots["inform_slots"][slot] = user_action["inform_slots"][
                    slot
                ]
                if slot in self.current_slots["request_slots"].keys():
                    del self.current_slots["request_slots"][slot]
