...
class KBHelper:
    ...
    def fill_inform_slots(self, inform_slots_to_be_filled, current_slots):
        ...
        if dialog_config.auto_suggest == 1:
            print "Number of movies in KB satisfying current constraints: ", len(
                kb_results
            )
        ...
        for slot in inform_slots_to_be_filled.keys():
            ...
            if slot == "ticket" or slot == "taskcomplete":
                filled_in_slots[slot] = (
                    dialog_config.TICKET_AVAILABLE
                    if len(kb_results) > 0
                    else dialog_config.NO_VALUE_MATCH
                )
                continue
    ...
    def available_results_from_kb(self, current_slots):
        ...
        constrain_keys = filter(
            lambda k: k != "ticket"
            and k != "numberofpeople"
            and k != "taskcomplete"
            and k != "closing",
            constrain_keys,
        )
        constrain_keys = [
            k for k in constrain_keys if current_slots[k] != dialog_config.I_DO_NOT_CARE
        ]
        ...
        for id in self.movie_dictionary.keys():
            ...
            if len(
                set(constrain_keys).union(set(kb_keys))
                ^ (set(constrain_keys) ^ set(kb_keys))
            ) == len(constrain_keys):
                match = True
                for idx, k in enumerate(constrain_keys):
                    if (
                        str(current_slots[k]).lower()
                        == str(self.movie_dictionary[id][k]).lower()
                    ):
                        continue
                    else:
                        match = False
                if match:
                    self.cached_kb[query_idx_keys].append(
                        (id, self.movie_dictionary[id])
                    )
    ...
    def available_results_from_kb_for_slots(self, inform_slots):
        ...
        for movie_id in self.movie_dictionary.keys():
            ...
            for slot in inform_slots.keys():
                if (
                    slot == "ticket"
                    or inform_slots[slot] == dialog_config.I_DO_NOT_CARE
                ):
                    continue

                if slot in self.movie_dictionary[movie_id].keys():
                    if (
                        inform_slots[slot].lower()
    ...
    def database_results_for_agent(self, current_slots):
        ...
        database_results = self.available_results_from_kb_for_slots(
    ...
    def suggest_slot_values(self, request_slots, current_slots):
        ...
        for slot in request_slots.keys():
            ...
            values_counts = [
                (v, avail_values_dict[v]) for v in avail_values_dict.keys()
            ]
