    ...
    def print_function(self, agent_action=None, user_action=None):
        ...
        if agent_action:
            if isinstance(self.user, RealUser) and self.agent.__class__.__name__ != "AgentCmd":
                ...
                if self.agent.__class__.__name__ != "AgentCmd":
                    print (
                        "Turn %d sys: %s, inform_slots: %s, request slots: %s"
                        % (
                            agent_action["turn"],
                            agent_action["diaact"],
            ...
            if dialog_config.auto_suggest == 1:
                print (
                    "(Suggested Values: %s)"
                    % (
                        self.state_tracker.get_suggest_slots_values(
                            agent_action["request_slots"]
                        )
                    )
                )
            ...
            if dialog_config.run_mode == 0:
                ...
                print (
                    "Turn %s usr: %s, inform_slots: %s, request_slots: %s"
                    % (
                        user_action["turn"],
                        user_action["diaact"],
                ...
                print (
                    "Turn %d usr: %s, inform_slots: %s, request_slots: %s"
                    % (
                        user_action["turn"],
                        user_action["diaact"],
                        user_action["inform_slots"],
                ...
                if len(user_request_slots) > 0:
                    ...
                    for slot in possible_values.keys():
                        if len(possible_values[slot]) > 0:
                            print (
                                "(Suggested Values: %s: %s)"
                                % (slot, possible_values[slot])
                            )
                        elif len(possible_values[slot]) == 0:
                            print (
                                "(Suggested Values: there is no available %s)" % (slot)
                            )
                else:
                    kb_results = self.state_tracker.get_current_kb_results()
                    print (
                        "(Number of movies in KB satisfying current constraints: %s)"
                        % len(kb_results)
                    )
