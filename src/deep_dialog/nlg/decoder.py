...
class decoder:
        ...
        for p in self.update:
            if p in grads:
                if sdg_type == "vanilla":
                    ...
                    self.step_cache[p] = (
                        self.step_cache[p] * decay_rate
                        + (1.0 - decay_rate) * grads[p] ** 2
                    )
                    dx = -(learning_rate * grads[p]) / np.sqrt(
                        self.step_cache[p] + smooth_eps
                    )
    ...
    def eval(self, ds, split, params):
        ...
        for i, ele in enumerate(ds.split[split]):
            ...
            log_perplex = 0
            log_perplex += -np.sum(
                np.log2(smooth_cost + probs[range(len(labels)), labels])
            )
            log_perplex /= len(labels)

            loss_cost = 0
            loss_cost += -np.sum(
                np.log(smooth_cost + probs[range(len(labels)), labels])
            )
    ...
    def predict(self, ds, split, params):
        inverse_word_dict = {
            ds.data["word_dict"][k]: k for k in ds.data["word_dict"].keys()
        }
        for i, ele in enumerate(ds.split[split]):
            pred_ys, pred_words = self.forward(
                inverse_word_dict, ele, params, predict_model=True
            )
            ...
            if params["dia_slot_val"] == 2 or params["dia_slot_val"] == 3:
                sentence = self.post_process(
                    sentence, ele["slotval"], ds.data["slot_dict"]
                )
