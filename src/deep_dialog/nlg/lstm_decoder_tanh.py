...
class lstm_decoder_tanh(decoder):
    ...
    def forward(self, dict, Xs, params, **kwargs):
        ...
        time_stamp = 0
        while True:
            if dict[pred_y_index] == "e_o_s" or time_stamp >= max_len:
                break
    ...
    def beam_forward(self, dict, Xs, params, **kwargs):
        ...
        for ele in beam_candidate_t:
            beams.append(
                (np.log(probs[0][ele]), [ele], [dict[ele]], Hout[0], Cellin[0])
            )
        ...
        while True:
            ...
            for b in beams:
                ...
                if decoder_sampling == 0:  # no sampling
                    ...
                    beam_candidate_t = np.random.choice(
                        Y.shape[1], beam_size, p=probs[0]
                    )
                # beam_candidate_t = (-probs[0]).argsort()[:beam_size]
                for ele in beam_candidate_t:
                    beam_candidates.append(
                        (
                            log_prob + np.log(probs[0][ele]),
                            np.append(b[1], ele),
                            np.append(b[2], dict[ele]),
                            hout_prev,
                            cell_in,
                        )
                    )
